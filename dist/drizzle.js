...
import { CheckBuilder, check, customType } from "drizzle-orm/pg-core";
...
function jsonSchema(schema) {
	...
	return Object.assign(column, { check(constraintName, col) {
...
var JsonbCheckCaseBuilder = class JsonbCheckCaseBuilder extends CheckBuilder {
	#column;
	#caseColumn;
	#whenConditions;
	...
	when(condition, schema) {
		...
		return new JsonbCheckCaseBuilder(this.name, this.#column, this.#caseColumn, [...this.#whenConditions, [condition, schema]]);
	}
};
var JsonbCheckBuilder = class extends CheckBuilder {
	#column;
...
function jsonbCheck(constraintName, col, schema) {
	if (!schema) return new JsonbCheckBuilder(constraintName, col);
